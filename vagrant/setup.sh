...
## Link Codebase
if [[ ! -d ~/bench-deployer/ ]]; then
    ln -s /vagrant ~/bench-deployer
fi
...
cd ~/bench-deployer
./sidechain.sh install-node --config "$CONFIG_PATH" --autoinstall-deps --non-interactive
./sidechain.sh install-explorer --config "$CONFIG_PATH" --skip-deps --non-interactive
...
~/bench-deployer/sidechain.sh start-node --name "$CHAIN_NAME" &>> ~/node.log &
~/bench-deployer/sidechain.sh start-explorer &>> ~/explorer.log &
EOS
...
echo "  Node API: http://127.0.0.1:6620/api/"
echo "  Explorer: http://127.0.0.1:4200/"
sudo reboot
